...
def load_gt(
    nusc: NuScenes, eval_split: str, box_cls, verbose: bool = False
) -> EvalBoxes:
    """
...
def filter_eval_boxes(
    ...
    :param min_dist: Maps the detection name to the eval distance threshold for that class.
    :param keywords: List of keywords to filter on. If None, all boxes pass.
    ...
    total, dist_filter, point_filter, bike_rack_filter, keyword_filter = 0, 0, 0, 0, 0
    for ind, sample_token in enumerate(eval_boxes.sample_tokens):
        ...
        if keywords is not None:
            ...
            scene_description = set(map(lambda x:x.strip(), scene_description.split(",")))
            if not scene_description & set(keywords):
                keyword_filter += len(eval_boxes[sample_token])
        ...
        eval_boxes.boxes[sample_token] = [
            ...
            and box.ego_dist > min_dist[box.__getattribute__(class_field)]
        ]
    ...
    if verbose:
        ...
        print("=> After keyword filtering: %d" % keyword_filter)
