...
def main():
    ...
    if torch.cuda.device_count() < len(config.GPUS):
        logger.critical(f"Not enough available gpu! {torch.cuda.device_count()} < {len(config.gpus)}")
        raise RuntimeError(f"Not enough available gpu! {torch.cuda.device_count()} < {len(config.gpus)}")
    if len(config.GPUS):
